...
// State variables
const router = args.params.router;
...
const getInitialJSON = () => {
  ...
  try {
    ...
    if (typeof queryParam === 'string') {
      ...
      code.value = decodedParam
      return decodedParam;
    }
    
    // Si c'est déjà un objet (Vue Router peut parser automatiquement)
    const strObj = JSON.stringify(queryParam, null, 2);
    code.value = strObj
    return strObj;
  } catch (error) {
...
getInitialJSON();
watch(router.currentRoute.value.query.json, getInitialJSON);
...
const convert = async () => {
  try {
    ...
    const response = await callServer(endpoint, repairedJSON.value);
    if (convertFormat.value === 'yaml') {
      convertedOutput.value = response.data;
    } else if (convertFormat.value === 'xml') {
      convertedOutput.value = response.data;
    }
