...
const generate = async () => {
  const { data } = await callServer('generateObjectId');
  uuid.value = data;
};
...
watchEffect(async () => {
  ...
  try {
    const { data } = await callServer('isValidObjectId', validateObjectId.value);
...
watchEffect(async () => {
  ...
  try {
    const { data } = await callServer('decodeObjectId', decodeObjectId.value);
...
watchEffect(async () => {
  ...
  try {
    const { data } = await callServer('objectIdFromDate', dateInput.value);
    dateObjectId.value = data;
  } catch (error) {
...
const compareObjectIds = async () => {
  ...
  try {
    const { data } = await callServer('compareObjectIds', compareId1.value, compareId2.value);
...
const testConnection = async () => {
  ...
  try {
    const { data } = await callServer('testConnection', connectionString.value);
