...
const socket = args.params.socket;
const SectionCmp = args.params.components.Section;
const Editor = args.params.components.Editor;
...
onMounted(async () => {
  socket.on('repl:update', handleSocketUpdate);
  await reload();
});
...
async function write() {
  try {
    ...
    await callServer('executeScript', room.value, code.value);
  } catch (error) {
...
async function formatCode() {
  try {
    const {data} = await callServer('formatCode', code.value);
...
async function reload() {
  try {
    const {data: _rooms} = await callServer('getRooms');
    rooms.value = _rooms;
    if (!room.value) {
      await changeRoom(rooms.value?.[0]);
    }
    const _room = await callServer('getRoom', room.value);
...
async function deleteRoom(_room) {
  try {
    await callServer('deleteRoom', _room);
    await reload();
  } catch (error) {
...
async function newRoom(room) {
  try {
    await callServer('createRoom', room);
    await reload();
...
async function loadEnvVars() {
  try {
    const { data } = await callServer('getEnvironmentVariables', room.value);
    envVars.value = data;
  } catch (error) {
...
async function addEnvVar() {
  ...
  try {
    ...
    await callServer('saveEnvironmentVariables', room.value, updatedEnv);
...
async function loadPackages() {
  try {
    const {data} = await callServer('getPackages', room.value);
...
async function installPackage() {
  ...
  try {
    ...
    await callServer('installPackage', packageSpec, room.value);
    ...
    if (errorMessage.includes('npm ERR! 404')) {
      packageError.value = `Package "${newPackageName.value}" not found on npm`;
    } else {
      packageError.value = errorMessage || 'Error installing package';
    }
  }
}
...
async function removePackage(name) {
  try {
    await callServer('uninstallPackage', room.value, name);
    await loadPackages();
  } catch (error) {
