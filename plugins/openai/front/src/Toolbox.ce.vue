...
const notification = args.params.notification;
...
async function reload() {
  const {data: _ready} = await callServer('isReady');
  ready.value = _ready;
  ...
  const {data: _messages} = await callServer('getChatMessages', room.value);
...
async function getModels() {
  const {data: _models} = await callServer('getModels');
...
async function sendMessage(message) {
  ...
  if (model.value.startsWith('dall')) {
    await callServer('generateImage', room.value, message, quality.value, resolution.value)
      .catch((err) => {
        console.error(err);
    ...
    await callServer('sendMessage', room.value, message, model.value, temperature.value)
      .catch((err) => {
        console.error(err);
        notification.next('error', 'Une erreur s\'est produite');
      })
