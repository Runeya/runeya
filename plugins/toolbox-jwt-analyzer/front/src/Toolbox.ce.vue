...
const decodeToken = async () => {
  ...
  try {
    const { data } = await callServer('decode', token.value);
    decodedToken.value = data;
    console.log('decodeToken', token.value, data)
...
const analyzeToken = async () => {
  ...
  try {
    const { data } = await callServer('analyze', token.value);
...
const verifyToken = async () => {
  ...
  try {
    ...
    const { data } = await callServer('verify', token.value, secret.value, options);
...
const generateToken = async () => {
  try {
    ...
    const { data } = await callServer('generate', payload, generateSecret.value, options);
    
    if (data) {
      generatedToken.value = data;
      generateError.value = '';
      saveToHistory(data, 'generated');
    } else if (data.error) {
