<template>
  <div class="base64-tool">
    ...
    <!-- Confirmation Dialog -->
    <Dialog v-model:visible="clearHistoryDialog" appendTo="self" header="Clear History" :modal="true" :closable="false" :style="{ width: '400px' }" :closeOnEscape="false">
...
async function processInput() {
  ...
  try {
    const endpoint = mode.value === 'encode' ? 'encode' : 'decode';
    const { data } = await callServer(endpoint, inputValue.value);
